...
const AutoClearSelection: React.FC<AutoClearSelectionProps> = ({ value, onChange }) => {
  const options = [
    { value: -1, label: 'Never' },
    ...
    <div>
      <div className="flex flex-row items-center px-4">
        <InfoTooltip type="AUTO_CLEAR" content={TOOLTIP_CONTENT.AUTO_CLEAR}>
          <Label className="mr-2 cursor-help text-muted-foreground" htmlFor="autoClearSelection">
            Auto-Clear
          </Label>
        </InfoTooltip>
        <Select onValueChange={(selectedValue) => onChange(parseInt(selectedValue))} value={value.toString()}>
